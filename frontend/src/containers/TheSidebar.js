  ...
  return (
    ...
    >
      ...
      <CSidebarNav>
        ...
        <ul style={{listStyleType:"none", margin:'1em 0', padding:0}} className="ul-sidebar">      
          {navigation.map(el => {
            let roleFilter = role.filter(element => el.permission?.includes(element))
              return (
                <> 
                  <h6 style={{opacity:0.5, padding:'0 20px'}}>{el._children}</h6>
                {roleFilter.length ? 
                <> 
                
                <li style={{padding:"10px 0 ", cursor:'pointer',padding:"10px 20px"}} onClick={()=>history.push(el.to)}>{el.name}</li>
                </>
                : null }
              </>)
          })}
