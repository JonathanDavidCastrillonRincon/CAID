...
export default function Calendar() {
  ...
  const print = () => {
    ...
    const dataFilter = activities.filter((el) => el.activityType === "1");
  ...
  const renderEventContent = (eventInfo) => {
    ...
    return (
      <>
        <b style={{ color: isExistObservation && "yellow" }}>
          {eventInfo.timeText}
        </b>
        &nbsp;
        <span style={{ color: isExistObservation && "yellow" }}>
          {eventInfo.event.title}
        </span>
        &nbsp;
        {isExistObservation && (
          <span style={{ color: "black" }}>
            (Se ha generado una observacion)
          </span>
        )}
      </>
    );
